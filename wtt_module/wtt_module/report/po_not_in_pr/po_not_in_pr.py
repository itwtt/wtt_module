...
def get_data(data, filters):
    data=[]
    poi=[]
    query = frappe.db.sql("""SELECT distinct(purchase_order_item)as purchase_order_item FROM `tabPurchase Receipt Item` WHERE docstatus!=2 and purchase_order_item IS NOT NULL """,as_dict=1)
    for i in query:
        poi.append(i.purchase_order_item)
    frappe.msgprint(str(poi))
