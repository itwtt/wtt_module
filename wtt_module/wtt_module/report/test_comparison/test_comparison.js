...
frappe.query_reports["Test Comparison"] = {
		"filters": [
		{
			fieldtype: "Link",
	...
	formatter: (value, row, column, data, default_formatter,datum) => {
		// alert(datum.grand_total)
		var ars=[]
		value = default_formatter(value, row, column, data);
			if(data['item_code']=='Basic Total')
			{
			...
			if(frappe.query_report.data[parseInt(v[i])].item_code!=undefined)
		 	{
				...
				'fields': [
				{'fieldtype': 'Select','label': 'Item Code', 'fieldname': 'item_code','options':opy},
				{"fieldtype": "Select", "label":"Supplier Quotation", "fieldname": "sq",'options':''},
				{"fieldtype": "Select", "label":"Supplier", "fieldname": "supplier",'options':''},
				...
				{"fieldtype": "Float", "label":"Rate Inr","fieldname":"rate_inr",'default':0},
				{"fieldtype": "Float", "label":"Amount Inr","fieldname":"amount_inr",'default':0},
				...
				// d.fields_dict.supplier.$input.on("change", function(event){
				// 	var sup = this.value;
				
				// 	frappe.call({
				// 	"method":"wtt_module.wtt_module.report.test_comparison.test_comparison.supplier_name",
				// 	args:{
				// 		"sq":sup
				// 	},
				// 	callback: function(r) {
				...
				frappe.call({
				"method":"wtt_module.wtt_module.report.test_comparison.test_comparison.fun_make2",
				args:{
					"sq":sqi,
					"itc":itc
				},
		...
		frappe.query_report.page.add_inner_button(__("Overall Quotation Approve"), function() {
			var selected_rows = [];
			...
			if(frappe.query_report.data[v[0]].item_code!=undefined)
		 	{
				...
				callback: function(r) {
					report.refresh();
				frappe.call({
				"method":"wtt_module.wtt_module.report.test_comparison.test_comparison.fun12",
				args:{
		...
		frappe.query_report.page.add_inner_button(__("Individual Item Reject"), function() {
			var selected_rows = [];
			var opy=[]
			var v=frappe.query_report.datatable.rowmanager.getCheckedRows();
			var rfq = frappe.query_report.get_filter_value('request_for_quotation')
