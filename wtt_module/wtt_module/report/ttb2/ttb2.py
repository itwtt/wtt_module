...
def get_data(data, filters):
	...
	if(filters.pending_emp==1):
		...
		ar1=[]
		ar2=[]
		for i in frappe.db.sql("SELECT name FROM `tabEmployee` WHERE status='Active'  and department!='House keeping - WTT' and name!='WTT001' ",as_dict=1):
			list1.append(i.name)
		for i in frappe.db.sql("SELECT DISTINCT(employee) FROM `tabTechnical Criteria` WHERE month=%(month)s ",filters,as_dict=1):
			list2.append(i.employee)
		for i in frappe.db.sql("SELECT DISTINCT(employee) FROM `tabBehavioural Criteria` WHERE month=%(month)s ",filters,as_dict=1):
			list3.append(i.employee)
		column1={}
		column2={}
		for j in list1:
			if j not in list2:
				ar1.append(frappe.db.get_value("Employee",str(j),"employee_name"))
		for ju in list1:
			if ju not in list3:
				ar2.append(frappe.db.get_value("Employee",str(ju),"employee_name"))
			...
			if(query or query2):
				...
				for beh in query2:
					if(i.employee_name==beh.employee):
						...
						column["total"]=round(((beh.attitude+beh.personal+beh.team+beh.skill+beh.knowledge+beh.performance)/6),2)
						...
						hod_row["total"]=round(((beh.attitude1+beh.personal1+beh.team1+beh.skill1+beh.knowledge1+beh.performance1)/6),2)
						...
						manag_row["total"]=round(((beh.attitude2+beh.personal2+beh.team2+beh.skill2+beh.knowledge2+beh.performance2)/6),2)
				data.append(column)
				data.append(hod_row)
