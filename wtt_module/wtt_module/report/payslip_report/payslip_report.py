...
def execute(filters=None):
	...
	for ss in salary_slips:
		row = [ss.employee, ss.employee_name,ss.leave_without_pay,ss.late_deduction,ss.actual_payment_days,ss.payment_days,ss.loss_of_pay,ss.sundays,ss.approval_leave,ss.punched_days]
		...
		if currency == company_currency:
			row += [flt(ss.total_deduction) * flt(ss.exchange_rate), flt(ss.net_pay) * flt(ss.exchange_rate),flt(ss.rounded_total) * flt(ss.exchange_rate)]
		else:
...
def get_columns(salary_slips):
	...
	columns = [
		...
		_("Leave Without Pay") + ":Float:50",_("Late Deduction") + ":Float:120",_("Actual Payment Days") + ":Float:120", _("Payment Days") + ":Float:120", _("Loss of Pay") + ":Float:120", _("Week off") + ":Float:120", _("Benefit Leave") + ":Float:120",_("Punched Days") + ":Float:120"
	]
	...
	columns = columns + [(e + ":Currency:120") for e in salary_components[_("Earning")]] + \
		...
		[_("Loan Repayment") + ":Currency:120", _("Total Deduction") + ":Currency:120", _("Net Pay") + ":Currency:120",_("Rounded Total") + ":Currency:120"]
...
def get_ss_earning_map(salary_slips, currency, company_currency):
	ss_earnings = frappe.db.sql("""select sd.parent, sd.salary_component, sd.amount, ss.exchange_rate, ss.name
		from `tabSalary Detail` sd, `tabPayslip` ss where sd.parent=ss.name and sd.parent in (%s)""" %
		(', '.join(['%s']*len(salary_slips))), tuple([d.name for d in salary_slips]), as_dict=1)
