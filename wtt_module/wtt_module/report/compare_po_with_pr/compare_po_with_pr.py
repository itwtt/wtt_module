...
def get_data(filters):
	...
	query = (
		...
		.select(
			...
			po_item.idx.as_("po_row"),
            pr_item.parent.as_("purchase_receipt"),
            pr_item.idx.as_("pr_row"),
            po.total_advance.as_("advance_paid"),
            po.base_rounded_total.as_("rounded_amount"),
			# po.status,
			po.supplier,
			po_item.item_code,
			po_item.description,
			po_item.technical_description,
			po_item.qty,
	...
	for field in ("company", "name","supplier"):
