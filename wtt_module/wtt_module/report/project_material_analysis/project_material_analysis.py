...
def get_data(conditions,data, filters):
	...
	for i in frappe.db.sql("""SELECT mr.transaction_date,mri.item_code,mri.description,mri.item_group,mri.technical_description,mri.idx,mri.qty,mri.parent,mr.workflow_state,mri.name as "ref_na" FROM `tabMaterial Request Item` as mri INNER JOIN `tabMaterial Request` as mr ON mr.name=mri.parent WHERE mr.workflow_state!="Cancelled" and mr.workflow_state!="Rejected"{conditions}""".format(conditions=conditions), as_dict=1):
		...
		po_date = frappe.db.get_value('Purchase Order',poi_no, 'transaction_date')
		if(poi_status == 'Cancelled' or poi_status == 'Rejected'):
			...
			d_from = frappe.db.get_value('Purchase Order Item', {'material_request_item': i.ref_na}, 'delivery_from')
			d_to = frappe.db.get_value('Purchase Order Item', {'material_request_item': i.ref_na}, 'delivery_to')
		...
		pri_qty = frappe.db.get_value('Purchase Receipt Item', {'material_request_item': i.ref_na}, 'qty')
		...
		pr_date = frappe.db.get_value('Purchase Receipt',pr_n, 'posting_date')
		...
		po_pend = '-'
		if(poi_status != 'Approved' and po_date is not None):
			d1 = datetime.strptime(str(po_date), "%Y-%m-%d")
			d2 = datetime.strptime(str(date.today()), "%Y-%m-%d")
			delta = d2 - d1
			po_pend = delta.days

		pr_pend = '-'
		if(pri_status == 'Draft' and pr_date is not None):
			d1 = datetime.strptime(str(pr_date), "%Y-%m-%d")
			d2 = datetime.strptime(str(date.today()), "%Y-%m-%d")
			delta = d2 - d1
			pr_pend = delta.days
		...
		d_pend = 0
		if(d_to is not None):
			d1 = datetime.strptime(str(d_to), "%Y-%m-%d")
			d2 = datetime.strptime(str(date.today()), "%Y-%m-%d")
			delta = d2 - d1
			d_pend = delta.days

		if(d_pend>0):
			vs = '<b style="color:red">'+str(d_pend)+'</b>'
		else:
			if(d_pend!=0):
				vs = '<b style="color:green">'+str(d_pend * -1)+'</b>'
			else:
				vs = '<p>'+str(d_pend)+'</p>'

		mr_flow = i.workflow_state
		if(mr_flow == 'Approved'):
			mr_flow = '<b style="color:green">'+str(mr_flow)+'</b>'
		elif(mr_flow == 'Approved by HOD'):
			mr_flow = '<b style="color:orange">'+str(mr_flow)+'</b>'
		else:
			if(mr_flow is None):
				mr_flow='-'
			else:
				mr_flow = '<b style="color:red">'+str(mr_flow)+'</b>'


		if(po_st == 'Approved'):
			po_st = '<b style="color:green">'+str(po_st)+'</b>'
		elif(po_st == 'Approved by HOD'):
			po_st = '<b style="color:orange">'+str(po_st)+'</b>'
		else:
			if(po_st is None):
				po_st='-'
			else:
				po_st = '<b style="color:red">'+str(po_st)+'</b>'

		if(pr_st == 'Completed'):
			pr_st = '<b style="color:green">'+str(pr_st)+'</b>'
		elif(pr_st == 'Draft'):
			pr_st = '<b style="color:red">'+str(pr_st)+'</b>'
		elif(pr_st == 'To Bill'):
			if(pr_st is None):
				pr_st='-'
			else:
				pr_st = '<b style="color:orange">'+str(pr_st)+'</b>'
		else:
			pr_st=pr_st

		freeze_no = frappe.db.get_value('Freeze Item Table', {'mr_ref': i.ref_na,'docstatus':1}, 'parent')
		freeze_idx = frappe.db.get_value('Freeze Item Table', {'mr_ref': i.ref_na,'docstatus':1}, 'idx')
		freeze_qty = frappe.db.get_value('Freeze Item Table', {'mr_ref': i.ref_na,'docstatus':1}, 'qty')
