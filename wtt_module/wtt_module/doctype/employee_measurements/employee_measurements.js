...
frappe.ui.form.on('Employee Measurements', {
	...
	validate:function(frm){
		var temp = frm.doc.employee_measurements_table;
		var i,sum=0
...
frappe.ui.form.on("Employee Measurements Table", {
	...
	end_time: function(frm, cdt, cdn) {
		...
		var hours = moment(child.end_time).diff(moment(child.start_time), "seconds") / 3600;
...
var calculate_end_time = function(frm, cdt, cdn) {
	...
	if(child.hours) {
		...
		if (std_working_hours < hours && std_working_hours > 0) {
			frappe.model.set_value(cdt, cdn, "hours", std_working_hours);
			frappe.model.set_value(cdt, cdn, "end_time", d.add(hours, "hours").format(frappe.defaultDatetimeFormat));
		} else {
