...
frappe.ui.form.on('Activity Sheet', {
	onload: function(frm){
		if (frm.is_new()) {
	...
	var set_employee_and_company = function(frm) {
	var options = { user_id: frappe.session.user };
	var fields = ['name', 'company'];
...
frappe.ui.form.on("Activity Sheet Table", {
