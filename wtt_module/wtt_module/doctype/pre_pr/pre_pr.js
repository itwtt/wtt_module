	...
	setup: (frm) => {
		frm.make_methods = {
			'Landed Cost Voucher': () => {
				...
				lcv_receipt.receipt_document_type = 'Pre PR';
	...
	refresh: function(frm) {
		...
		if (frm.doc.docstatus === 1 && frm.doc.is_return === 1 && frm.doc.per_billed !== 100) {
			frm.add_custom_button(__('Debit Note'), function() {
				frappe.model.open_mapped_doc({
					method: "wtt_module.wtt_module.doctype.pre_pr.pre_pr.make_purchase_invoice",
					frm: cur_frm,
				})
			}, __('Create'));
		...
		if (frm.doc.docstatus === 1 && frm.doc.is_internal_supplier && !frm.doc.inter_company_reference) {
			frm.add_custom_button(__('Delivery Note'), function() {
				frappe.model.open_mapped_doc({
					method: 'wtt_module.wtt_module.doctype.pre_pr.pre_pr.make_inter_company_delivery_note',
					frm: cur_frm,
				})
			}, __('Create'));
		}
...
erpnext.stock.PrePRController = class PrePRController extends erpnext.buying.BuyingController {
	setup(doc) {
			...
			if (this.frm.doc.docstatus == 0) {
				this.frm.add_custom_button(__('Purchase Order'),
					function () {
						...
						erpnext.utils.map_current_doc({
							method: "wtt_module.wtt_module.doctype.pre_pr.pre_pr.make_purchase_receipt",
	...
	make_purchase_return() {
		frappe.model.open_mapped_doc({
			method: "wtt_module.wtt_module.doctype.pre_pr.pre_pr.make_purchase_return",
			frm: cur_frm
		})
	}
		...
		frappe.call({
			...
			callback: function (r) {
				if (r.message) {
					...
					frappe.msgprint(__("Pre PR doesn't have any Item for which Retain Sample is enabled."));
				}
			}
		});
	}
...
extend_cscript(cur_frm.cscript, new erpnext.stock.PrePRController({frm: cur_frm}));

cur_frm.cscript.update_status = function(status) {
	frappe.ui.form.is_saving = true;
	frappe.call({
		method:"wtt_module.wtt_module.doctype.pre_pr.pre_pr.update_purchase_receipt_status",
...
frappe.ui.form.on("Pre PR", "is_subcontracted", function(frm) {
...
cur_frm.cscript['Make Stock Entry'] = function() {
	frappe.model.open_mapped_doc({
		method: "wtt_module.wtt_module.doctype.pre_pr.pre_pr.make_stock_entry",
		frm: cur_frm,
	})
}
