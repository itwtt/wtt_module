...
class PrePR(BuyingController):
	def __init__(self, *args, **kwargs):
		super(PrePR, self).__init__(*args, **kwargs)
		self.status_updater = [
			{
				...
				[
					{
						...
						"second_join_field": "po_detail",
						"extra_cond": """ and exists (select name from `tabPre PR`
						where name=`tabPre PR Item`.parent and is_return=1)""",
						"second_source_extra_cond": """ and exists (select name from `tabPurchase Invoice`
						where name=`tabPurchase Invoice Item`.parent and is_return=1 and update_stock=1)""",
					},
					{
						"source_dt": "Pre PR Item",
						"target_dt": "Pre PR Item",
						"join_field": "purchase_receipt_item",
						"target_field": "returned_qty",
						"target_parent_dt": "Pre PR",
	...
	def validate_with_previous_doc(self):
		super(PrePR, self).validate_with_previous_doc(
			{
				"Purchase Order": {
	...
	def get_already_received_qty(self, po, po_detail):
		qty = frappe.db.sql(
			"""select sum(qty) from `tabPre PR Item`
	...
	def add_provisional_gl_entry(
		...
		if reverse:
			...
			expense_account = frappe.db.get_value(
				"Pre PR Item", {"name": item.get("pr_detail")}, ["expense_account"]
			)
	...
	def update_billing_status(self, update_modified=True):
		...
		for pr in set(updated_pr):
			pr_doc = self if (pr == self.name) else frappe.get_cached_doc("Pre PR", pr)
...
def update_billed_amount_based_on_po(po_details, update_modified=True):
	...
	# Get all Pre PR Item rows against the Purchase Order Items
	...
	for pr_item in pr_details:
		...
		# Get billed amount directly against Pre PR
...
def get_purchase_receipts_against_po_details(po_details):
	# Get Pre PRs against Purchase Order Items

	purchase_receipt = frappe.qb.DocType("Pre PR")
	purchase_receipt_item = frappe.qb.DocType("Pre PR Item")
...
def get_item_wise_returned_qty(pr_doc):
	...
	return frappe._dict(
		frappe.get_all(
			"Pre PR",
			fields=[
				"`tabPre PR Item`.purchase_receipt_item",
				"sum(abs(`tabPre PR Item`.qty)) as qty",
			],
			filters=[
				["Pre PR", "docstatus", "=", 1],
				["Pre PR", "is_return", "=", 1],
				["Pre PR Item", "purchase_receipt_item", "in", items],
			],
			group_by="`tabPre PR Item`.purchase_receipt_item",
			as_list=1,
		)
	)
...
def make_purchase_invoice(source_name, target_doc=None):
	...
	doc = frappe.get_doc("Pre PR", source_name)
...
def get_returned_qty_map(purchase_receipt):
	...
	returned_qty_map = frappe._dict(
		...
		from `tabPre PR Item` pr_item, `tabPre PR` pr
...
def make_purchase_return(source_name, target_doc=None):
	...
	return make_return_doc("Pre PR", source_name, target_doc)


@frappe.whitelist()
def update_purchase_receipt_status(docname, status):
	pr = frappe.get_doc("Pre PR", docname)
...
def make_inter_company_delivery_note(source_name, target_doc=None):
	return make_inter_company_transaction("Pre PR", source_name, target_doc)
...
def on_doctype_update():
	frappe.db.add_index("Pre PR", ["supplier", "is_return", "return_against"])
...
@frappe.whitelist()
def make_purchase_receipt(source_name, target_doc="Pre PR"):
