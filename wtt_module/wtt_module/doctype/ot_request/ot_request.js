...
frappe.ui.form.on('OT request', {
	...
	validate: function(frm, cdt, cdn) {
		var nn=new Date(frm.doc.from_time).toLocaleString('en-us', {weekday:'long'})
		// var nn2=new Date(frm.doc.to_time).toLocaleString('en-us', {weekday:'long'})
		if(nn=='Sunday'){
			frappe.throw("No Need to apply OT for Sunday; It must be calculated on Attendance")
		}
	...
	from_time: function(frm, cdt, cdn) {
		var nn=new Date(frm.doc.from_time).toLocaleString('en-us', {weekday:'long'})
		if(nn=='Sunday'){
			msgprint("No Need to apply OT for Sunday; It must be calculated on Attendance")
		}
