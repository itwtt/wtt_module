...
class MFStandardItems(Document):
	pass
...
def get_mf(arr):
	...
	for i in to_python:
		if(str(i['flow'])=='0'):		
			...
			for k in frappe.db.sql("SELECT item_code,item_description,name,'Flow and Range are equal'as 'remarks' from `tabChild Fetch Items` where item_description='"+str(i['renamed'])+"' and flow>='"+str(i['flow'])+"' and flow<='"+str(addflow)+"'",as_dict=1):
