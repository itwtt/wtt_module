...
class SalesInvoice(SellingController):
	...
	def mail_pdf(self):
		...
		message=f"<html>Dear <b>"+str(self.customer)+"</b>,<br><br>Please find the Credit note against "+str(self.return_against)
		password = None
		if receiver:
			email_args = {
				"recipients": [receiver],
				"message": _(message),
				"subject": 'Credit Note against {0}'.format(str(self.return_against)),
				"attachments": [frappe.attach_print(self.doctype, str(self.name), file_name=str(self.name), password=password)],
			...
			msgprint(_("{0}: Supplier email not found, hence email not sent").format(self.customer))
