...
class PurchaseReceipt(BuyingController):
	...
	def create_task(elf):
		if(self.status=='Draft' and self.create_task==1):
			if(frappe.db.exists("Task Allocation", {"mr_inward": self.name})):
				...
				doc.save()
				frappe.db.commit()
				frappe.msgprint("Task Allocated for "+str(self.employee_name))


	def check_draft_receipt(self):
		ar=[]

		for i in frappe.db.sql("SELECT purchase_order_item,qty FROM `tabPurchase Receipt Item` WHERE parent!='"+str(self.name)+"' and docstatus=0 ",as_dict=1):
			ar.append(i)
		for j in self.items:
			if({"purchase_order_item":j.purchase_order_item,"qty":j.qty} in ar):
				frappe.throw(j.item_code+" is already received by another receipt yet in draft state")
	...
	def on_submit(self):
		...
		# 	if(i.item_inspection==None and self.is_return==0):
...
def make_purchase_invoice(source_name, target_doc=None):
	...
	def set_not_received_items(source, target):
		for i in source.not_received_items:
...
@frappe.whitelist()
def make_job_order_request(source_name, target_doc=None):
	doclist = get_mapped_doc("Job Order Request", source_name,	{
		"Job Order Request": {
			"doctype": "Stock Entry",
			"validation": {
				"docstatus": ["=", 1],
