...
class PurchaseInvoice(BuyingController):
	...
	def on_submit(self):
		...
		if(self.is_return==1 and self.send_mail==1):
			self.mail_pdf()
	...
	def mail_pdf(self):
		...
		if receiver:
			...
			msgprint(_("{0}: Supplier email not found, hence email not sent").format(self.supplier))
	...
	def get_not_received_items(self):
		...
		query = frappe.db.sql("SELECT * from `tabNot Received Items` where parent in '"+pr+"' and docstatus=1 ",as_dict=1)
		datas = []
		if query:
			for i in query:
				datas.append(i)
...
# @frappe.whitelist()
# def mail_pdf(doc):
# 	invoice = frappe.get_doc("Purchase Invoice",str(doc))
# 	receiver = "erp@wttindia.com"
# 	message=f"<html>Dear <b>"+str(invoice.supplier)+"</b>,<br><br>Please find the Debit note against "+str(invoice.supplier_delivery_note)
# 	password = None
# 	if receiver:
# 		email_args = {
# 			"recipients": [receiver],
# 			"message": _(message),
# 			"subject": 'Debit Note against {0}'.format(str(invoice.supplier_delivery_note)),
# 			"attachments": [frappe.attach_print("Purchase Invoice", str(invoice.name), file_name=str(invoice.name), password=password)],
# 			"reference_doctype": "Purchase Invoice",
# 			"reference_name": str(invoice.name)
# 			}
# 		if not frappe.flags.in_test:
# 			enqueue(method=frappe.sendmail, queue='short', timeout=300, is_async=True, **email_args)
# 		else:
# 			frappe.sendmail(**email_args)
# 	else:
# 		msgprint(_("{0}: Supplier email not found, hence email not sent").format(invoice.supplier))