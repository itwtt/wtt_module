	...
	def validate(self):
		for i in self.items:
			if(i.material_request):
				val=frappe.db.sql("UPDATE `tabMaterial Request Item` SET stock_status='Item Issued',stock_issued_qty='"+str(i.qty)+"' WHERE name='"+str(i.material_request_item)+"' AND parent='"+str(i.material_request)+"'")
		# frappe.msgprint('Material Status Updated')
